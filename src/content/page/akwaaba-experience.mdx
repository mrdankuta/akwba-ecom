---
title: The Akwaaba Experience
description: The Akwaaba Experience is about more than food; it's a vibrant
  celebration of Ghanaian culture, community, and flavor!
intro: The Akwaaba Experience is about more than food; it's a vibrant
  celebration of Ghanaian culture, community, and flavor!
hero_buttons:
  - icon_only: false
    label: Book Us Now
    href: "#"
blocks:
  - type: text_image
    component: TextImage
    container: full
    template: row
    title: My Image
    image_size: "50"
    reverse: false
    animate: false
    content: Imagine the spice and joy of Ghana infused into your next special
      occasion. We handle all cuisine and presentation while you enjoy the food,
      fun, and meaningful connections!
    thumbnail: /src/assets/akwba-exprnc.png
  - type: gallery
    component: ImageGallery
    title: Gallery
    container: md
    animate: false
    images:
      - image: /src/assets/akwaaba-foods-229.webp
        title: Title
      - image: /src/assets/akwaaba-foods-39.jpg
        title: galla
style:
  template: full
  hero_template: plain
  hero_image_opacity: ""
  container: md
thumbnail: /src/assets/akwaaba-foods-229.webp
---
